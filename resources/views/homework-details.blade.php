    ...
    <div class="row">
        <div class="col-6">
                ...
                <div class="card text-center">
                    <div class="card-body">
                        <a class="btn btn-info"
                           href="{{ url('/homework/' . $homework->slug . '/edit') }}">Editeaz&#259;</a>
                    </div>
                </div>
            @endif
            ...
            @if (can_subscribe($homework->course->id))
                <div class="card text-center">
                    <a href="{{ url('/course/' . $homework->course->slug) }}" class="btn btn-primary">Aboneaz&#259;-te
                        la curs</a>
                </div>
            @endif
        ...
        @if($homework->deadline <= Carbon\Carbon::now() || !isAlreadyMarked($homework))
            <div class="col-6">
                <div class="mb-0 mt-4">
                    <i class="fa fa-newspaper-o"></i> Incarcare tema
                </div>
                <hr class="mt-2">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <div class="custom-file">
                                        <input name="toUpload[{{ $counter }}][upload_file]" type="file"
                                               class="custom-file-input">
                                        <input name="toUpload[{{ $counter }}][requirement_id]" type="hidden"
                                               value="{{ $requirement->id }}">
                                        <label class="custom-file-label" for="toUpload{{ $counter }}">Incarca</label>
                                    </div>
                                </div>
                                <div class="card-footer">Formatul
                                    necesar: {{ $requirement->format->extension_name }}</div>
                            </div>
                            <?php $counter++; ?>
                        @endforeach
                    </div>
